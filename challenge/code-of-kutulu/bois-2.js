...
function Terrain(width, height) {
    ...
    /**
     * affiche la map en temps réel
     */
    this.printRealTime = function () {
        this.buffMapping.map(x => printErr(x.reduce((a, b) => a + b)));
    }
    /**
     * met la map à jour au tour par tour
     * @param {int}    x          position x
     * @param {int}    y          position y
     * @param {string} entityType type d'unité
     * @param {int}    i          position dans le tableau des unités
     */
...
function action() {
    ...
    if (distance(me, ennemie) <= 3) {
        let oppY = parseInt((me.x - ennemie.x) / 3),
            oppX = parseInt((me.y - ennemie.y) / -3),
            test = 0,
            alentoure = map.buffMapping[me.y + oppY][me.x + oppX],
            freespot = ['.', '|'];
        while (freespot.indexOf(alentoure) === -1) {
            let buffOpp = oppX;
            alentoure = map.buffMapping[me.y + oppY][me.x + oppX];
            oppX = oppY;
            oppY = -buffOpp;
